...
Below is the complete, high-resolution tensor transformation pipeline of a standard CNN designed to classify a $224 \times 224 \times 3$ RGB input image into one of 10 target classes:
...
* **Role:** Applied immediately after every convolution layer to introduce non-linearity, enabling the network to learn complex, non-linear patterns.
* **Mathematical Formula:**
  $$f(x) = \max(0, x)$$
...
* **Mechanism:** A traditional feedforward neural network layer where every neuron is connected to every activation in the previous flattened layer. It learns complex, non-linear combinations of features:
...
### 📈 4. The Principle of Hierarchical Feature Extraction
A key feature of deep CNNs is that they learn features hierarchically across layers, automatically transitioning from local, concrete details to global, abstract concepts:
...
The **Convolutional Layer** is the core computational building block of a CNN. It acts as a local feature scanner that preserves spatial relationships. 
...
It is designed to model datasets where elements have hierarchical relationships, such as the syntactic parse trees of sentences in natural language processing or the nested hierarchies of computer code compiler syntax.
...
graph TD
    ...
    Prod --> Explode["Exploding Gradients<br/>If largest eigenvalue of W_hh > 1<br/>Gradients grow exponentially<br/>Causes weight oscillations & NaN errors"]
